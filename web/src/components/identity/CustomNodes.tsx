...
export const CustomIdentityNode: React.FC<any> = memo(({ data, selected }: any) => {
  ...
  const styleMap: Record<IdentityNodeType, { border: string; bg: string; text: string; iconBg: string }> = {
    ...
    secret: {
      border: 'border-critical shadow-glow-critical/40 animate-pulse',
      bg: 'bg-critical/10',
  ...
  return (
    <div
      className={`min-w-[180px] max-w-[240px] rounded-xl border p-3 shadow-card backdrop-blur-md transition-all duration-200 ${
        style.bg
    ...
    >
      ...
      <div className="flex items-center gap-2.5">
        <div className={`flex h-8 w-8 items-center justify-center rounded-lg p-1.5 ${style.iconBg}`}>
