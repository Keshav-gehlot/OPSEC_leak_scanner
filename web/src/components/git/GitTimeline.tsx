        ...
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border pb-5">
          <div>
            <div className="flex items-center gap-2.5">
              <span className={`px-2.5 py-0.5 rounded-lg text-[11px] font-mono font-bold uppercase border ${
                selectedItem.currentStatus === 'ACTIVE_IN_HEAD'
                  ? 'bg-critical/15 text-critical border-critical/30'
                  : selectedItem.currentStatus === 'HISTORICAL_LEAK'
            ...
            <p className="text-xs text-text-muted mt-1 font-mono">
              Persisted across {selectedItem.commitsPersistent} commits • Discovered between {selectedItem.firstSeenDate} and {selectedItem.lastSeenDate}
            </p>
          </div>

          <div className="flex items-center gap-3 font-mono text-xs">
            <div className="rounded-xl border border-border bg-sidebar px-3 py-1.5">
            ...
            <div className="rounded-xl border border-border bg-sidebar px-3 py-1.5">
              <span className="text-text-muted text-[10px] block uppercase">Last Author</span>
              <span className="font-semibold text-text-primary">{selectedItem.lastAuthor}</span>
            </div>
          </div>
        </div>

        {/* Warning Banner for Historical Secrets */}
        {selectedItem.currentStatus === 'HISTORICAL_LEAK' && (
          <div className="rounded-xl border border-high/40 bg-high/10 p-4 flex items-start gap-3 animate-fade-in">
            <AlertTriangle className="h-5 w-5 text-high flex-shrink-0 mt-0.5" />
            <div className="text-xs">
              <p className="font-bold text-high">
                ⚠ This finding has been removed from HEAD but remains recoverable from Git history.
              </p>
              <p className="text-text-secondary mt-0.5 leading-relaxed">
                Attackers who clone or inspect the repository history can trivially checkout past commits or extract historical commit blob objects.
              </p>
            </div>
          </div>
        )}
            ...
            {selectedItem.timeline.map((event, idx) => {
              ...
              return (
                <div key={idx} className="relative group">
                  ...
                  <div className="rounded-xl border border-border bg-surface-elevated/70 p-4 transition-all hover:border-border/90 hover:bg-surface-elevated">
        ...
        <div className="rounded-xl border border-primary/40 bg-primary/10 p-5 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-xs font-bold uppercase tracking-wider text-primary font-mono">
                Permanent Git History Scrub Command
              </h4>
            </div>
          ...
          <div className="flex items-center justify-between rounded-lg border border-border bg-background px-4 py-2.5 font-mono text-xs text-text-primary">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1 rounded bg-surface hover:bg-surface-elevated border border-border text-[11px] text-text-secondary hover:text-text-primary transition-colors flex-shrink-0"
            >
